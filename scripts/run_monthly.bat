...
REM   schtasks /Create /SC MONTHLY /D 5 /TN "SenadoSentinel_Mensal" /TR "\"%~dp0run_monthly.bat\"" /ST 03:00
REM
...
"%UV%" run python -m senado_sentinel.cli run-all
set EXITCODE=%ERRORLEVEL%

if not %EXITCODE%==0 (
    echo [run_monthly.bat] Pipeline falhou com codigo %EXITCODE% - veja logs\senado_sentinel.log
)
