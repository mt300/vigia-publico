...
def render(mode: Mode, filtros: Filtros) -> None:
    ...
    if mode == "public":
        ...
        # "GASTO_OUTLIER_VS_PARES"), mas resolve de volta pro codigo antes
        # de consultar - ver detection/neutral_copy.py.
    ...
    findings = data.get_findings(
        filtros.mes_inicio, filtros.mes_fim, filtros.partidos, filtros.ufs, filtros.deputado_id, tipos_sel, severidades_sel
    )
    ...
    # "fonte" (tela) e "fonte_url" (export) sao intencionalmente diferentes
    # no modo publico: a tela usa o link amigavel (evita o visitante cair
    # no XML cru da API - ver fonte_page.py), o export CSV/Excel/Markdown
    # mantem a URL crua (mais direta pra quem vai processar o dado por
    # conta propria).
    if mode == "public":
        findings["resumo"] = [
    ...
    filtros_atuais = {
        ...
        "Deputado": filtros.deputado_label if filtros.deputado_id else None,
    }
    st.download_button(
